<?
	...
	$fin=$ini+5;
	$check=mysql_query("SELECT * FROM db_estudiantes WHERE '$search_crit' LIKE '%$search_text%' ORDER BY ap_1 ASC");
	$select=@mysql_query("SELECT * FROM db_estudiantes WHERE $search_crit LIKE '%$search_text%' ORDER BY ap_1 ASC  LIMIT $ini,$fin");
	...
	if($result==0)
	{
	echo "No se encontraron resultados que coincidan con <strong>'".$search_text."'</strong>.";
	}
	else
	{
